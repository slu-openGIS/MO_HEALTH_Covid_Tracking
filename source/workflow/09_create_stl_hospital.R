...
hosp_data %>%
  ...
  select(report_date, new_in_pt, new_in_pt_avg, in_pt, in_pt_avg, icu, icu_avg, 
         vent, vent_avg, mortality, mortality_avg) -> hosp_sub

# write data
write_csv(hosp_sub, file = "data/metro/stl_hospital.csv")

hosp_data %>%
  filter(report_date >= as.Date("2021-08-06")) %>%
  filter(is.na(peds_in_pt_0_11) == FALSE) %>%
  ...
  select(report_date,
         ...
         peds_icu, peds_icu_avg)  -> hosp_sub

# write data
write_csv(hosp_sub, file = "data/metro/stl_hospital_peds.csv")
